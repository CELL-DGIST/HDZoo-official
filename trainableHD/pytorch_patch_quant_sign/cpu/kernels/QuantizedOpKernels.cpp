...
// Yeseong
void qsign_kernel(const Tensor& qx, Tensor& qy) {
	// defines input and output scales and zero_points
	int64_t input_zero_point = qx.q_zero_point();
	// NOLINTNEXTLINE(bugprone-narrowing-conversions,cppcoreguidelines-narrowing-conversions)
	float input_scale = qx.q_scale();
    ...
    Vec input_zero_point_vec = Vec(input_zero_point);
    Vec input_scale_neg_zp_premul_vec = input_scale_vec * input_zero_point_vec.neg();
    Vec threshold_vec = Vec(0.0f);
    Vec one_vec = Vec(1.0f);
    Vec minus_one_vec = Vec(-1.0f);

    auto iter = TensorIterator::unary_op(qy, qx);
    cpu_kernel_vec(
        iter,
        [&](scalar_t value) -> scalar_t {
			if (value.val_ > input_zero_point)
				return at::native::quantize_val<scalar_t>(1.0f, 1, 1.0f);
			else if (value.val_ == input_zero_point)
				return at::native::quantize_val<scalar_t>(1.0f, 1, 0.0f);
			else
				return at::native::quantize_val<scalar_t>(1.0f, 1, -1.0f);
        },
        [&](qVec value_qx) -> qVec {
          ...
          for (auto & value : dx_vec) {
            ...
            if (cmp_to_threshold.zero_mask()) {
              // blend
              value = Vec::blendv(minus_one_vec, one_vec, cmp_to_threshold);
            }
            const auto cmp_to_threshold2 = value == threshold_vec;
            if (cmp_to_threshold2.zero_mask()) {
				value = Vec::blendv(value, threshold_vec, cmp_to_threshold2);
			}
          }
          // quantize
          return qVec::quantize(dx_vec, 1.0f, 1, 1.0f);
        });
  });
}
...
// Yeseong
REGISTER_DISPATCH(qsign_stub, &qsign_kernel);
